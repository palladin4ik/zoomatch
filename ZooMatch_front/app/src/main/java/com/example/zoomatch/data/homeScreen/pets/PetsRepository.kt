...
class PetsRepository(
  ...
  suspend fun fetchAndCachePet(petId: Int): Boolean {
    val result = dataSource.getPetById(tokenManager.getAccessToken(), petId)
    if (result is Result.Success) {
      val data = result.data
      val current = petDao.getPetById(petId)
      val updated = (current ?: PetEntity(
        id = data.id, name = data.name, animal_type_id = data.animal_type.id,
        breed_id = data.breed.id, is_male = data.is_male, age = data.age,
        owner_id = 0, avatar = null, location = data.location,
        has_pedigree = data.has_pedigree, pedigree_documents = data.pedigree_documents,
        awards = data.awards, description = data.description, is_active = data.is_active,
        animal_type_custom = null, breed_custom = null, moderation_status = null
      )).copy(
        name = data.name,
        ...
        awards = data.awards,
        pedigree_documents = data.pedigree_documents ?: current?.pedigree_documents,
        avatar = data.avatar?.toString() ?: current?.avatar,
        moderation_status = data.moderation_status ?: current?.moderation_status
      )
      petDao.insert(updated)
      return true
    }
